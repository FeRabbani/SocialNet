'''
In this script we create a network of Members based on Dark web data.
First, we calculate the network between members in each during 30 days, the shift of time step starts from the first comment and
connection between members happens during 30 days then time step shifts slowly to the second comment and calculation is done
...
#===============================================create dictionary for threads and members by time
...
for i in range(len(Date_list)):
    ...
    if MemberID[i] not in Dic_of_Date_Member_Thread[D]:
        Dic_of_Date_Member_Thread[D][MemberID[i]] = []
    
    Dic_of_Date_Member_Thread[D][MemberID[i]].append(ThreadID[i])
  



Date_Members_Threads={}
...
for x in range(0,250):
    ...
    for j in range(len(Dic_of_Date_Member_Thread)):
        ...
        if   Time <= Dic_of_Date_Member_Thread.keys()[j] < date2:
            ...
            Date_Members_Threads[Time].append(Dic_of_Date_Member_Thread.values()[j])
...
Date_Members_Threads= OrderedDict(sorted(Date_Members_Threads.items(), key=lambda t: t[0]))   

        
#===================================================create edges between two members if they commented on at least 2 same thread
Date_Edges={}
Date_Nodes={}

for i in range(len(Date_Members_Threads)):
    Date_Edges[Date_Members_Threads.keys()[i]]=[]
    Date_Nodes[Date_Members_Threads.keys()[i]]=[]
    for item in Date_Members_Threads.values()[i]:
        for k in range(len(item)):
            for l in range(k+1,len(item)):
                if len(set(item.values()[k]) & set(item.values()[l]))>=2:
                    
                    Date_Edges[Date_Members_Threads.keys()[i]].append([item.keys()[k],item.keys()[l],len(set(item.values()[k]) & set(item.values()[l]))])
                    
                    Date_Nodes[Date_Members_Threads.keys()[i]].append((item.keys()[k]))
                    Date_Nodes[Date_Members_Threads.keys()[i]].append((item.keys()[l])) 
...
with open('Gawaher_NodeMemberNetwork_EachMonth.csv','w') as out:
    csv_out=csv.writer(out)
    csv_out.writerow(['Id','Label','Timestamp'])
    for row in gephinode:
        csv_out.writerow(row)

with open('Gawaher_EdgeMemberNetwork_EachMonth.csv','w') as out:
    csv_out=csv.writer(out)
